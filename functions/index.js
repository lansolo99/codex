...
exports.sendNotificationsReminder = functions.pubsub
  .topic("weekx-reminders")
  .onPublish(message => {
    ...
    return admin
      ...
      =============================================*/
        ...
        users.forEach(doc => {
          ...
          if (
            doc.data().profile.notifications.token &&
            doc.data().profile.notifications.token !== ""
          ) {
            ...
            if (doc.data().tasks) {
              ...
              const userDefinedHour = doc.data().profile.notifications
                .dailyTaskReminder.time;
              const userPushTTL = Math.abs(
                differenceInSeconds(new Date(Date.now()), endOfToday())
              ).toString();
              ...
              console.log(
                "userConvertedToServerHour = " + userConvertedToServerHour
              );

              if (serverHour === userConvertedToServerHour) {
                console.info(
                  `a user has defined an hour that match the current one which is ${serverHour}`
                );
              }
              // End hour test
                  ...
                  let message = {
                    ...
                    webpush: {
                      ...
                      notification: {
                        ...
                        body: `You have ${countDailyTasks} task${plural} today!'`,
                        icon:
                          "https://app.weekx.xyz/img/icons/android-chrome-512x512.png",
                        click_action: "https://weekx.netlify.com",
                        badge:
                          "https://app.weekx.xyz/img/icons/notification-badge.png"
                      },
                      fcm_options: {
                        link: "https://app.weekx.xyz/"
                      }
                    }
                  };
